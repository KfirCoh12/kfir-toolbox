...
class ConnectionTests(unittest.TestCase):
    ...
    def test_public_iec_rating_series_used_by_catalog(self):
        ratings = [
            get_connection_option(f"industrial_{a}a_3ph").rating_a
            for a in (16, 32, 63, 125)
        ]
    ...
    def test_invalid_phase_is_rejected_in_lookup_and_suggestion(self):
        with self.assertRaises(ValueError):
            connection_options_for_phase("invalid")
        with self.assertRaises(ValueError):
            suggest_connection(phase="invalid", required_current_a=32)

    def test_non_finite_required_current_is_rejected(self):
        for bad in (math.nan, math.inf, -math.inf):
            with self.subTest(value=bad):
                with self.assertRaises(ValueError):
                    suggest_connection(phase="three", required_current_a=bad)
