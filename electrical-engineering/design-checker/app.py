...
st.markdown("""<div class="hero"><div class="eyebrow">Electrical engineering · V0.9</div><h1>⚡ Electrical Design Checker</h1><p>Size a new supply or find the usable capacity of electrical equipment you already have.</p></div>""", unsafe_allow_html=True)
...
if mode == "Design a supply":
    ...
    if st.button("Calculate existing capacity", type="primary", use_container_width=True):
        ...
        missing=[]
        if not use_breaker: missing.append("Breaker / upstream protection was not provided.")
        if not use_cable: missing.append("Cable ampacity was not provided or checked.")
        if not use_connection: missing.append("Outlet / connection rating was not provided.")
        if missing:
            with st.expander("Still needs verification"):
                for x in missing: st.write("•", x)
...
st.caption("V0.9 focuses on two practical questions: what supply should this load use, and what can the electrical equipment I already have support?")
