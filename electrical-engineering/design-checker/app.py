"""First local UI for the Electrical Design Checker V0.
...
st.caption("Single-feeder engineering checker. Results preserve IEC/source verification status; NOT VERIFIED is not a pass.")
...
with st.sidebar:
    ...
    insulation = st.selectbox("Insulation/data family", ["xlpe_epr", "other"], help="V0 IEC ampacity data currently supports generic XLPE/EPR only. Fire-rated product families must use a defensible product source before verification.")
    ...
    arrangement = None
    if grouped > 1:
        arrangement = st.selectbox("Grouping arrangement", ["bunched", "perforated_tray", "ladder"])
    parallel = st.number_input("Parallel runs", min_value=1, value=1, step=1)
    equal_share = None
    if parallel > 1:
        equal_share = st.checkbox("Acceptable/equal current sharing confirmed")
...
if run:
    ...
    feeder = FeederInput(
        load_type="kw", load_value=float(load_kw), voltage_v=float(voltage_v), phase=phase,
        power_factor=float(pf), demand_factor=float(demand), breaker_in_a=float(breaker_a), cable=cable,
    ...
    status_icon = {"PASS": "✅", "FAIL": "❌", "NOT VERIFIED": "⚠️"}.get(r.overall_outcome, "ℹ️")
    st.header(f"{status_icon} Overall: {r.overall_outcome}")
    if r.overall_outcome == "NOT VERIFIED":
        st.warning("One or more engineering/standards checks are incomplete or not currently verified. This is not an approval of the feeder.")
    ...
    with st.expander("Calculation trace"):
        st.markdown("**Current**")
        for line in r.current.calculation_trace: st.code(line)
        if r.breaker:
            st.markdown("**Breaker**")
            for line in r.breaker.calculation_trace: st.code(line)
        if r.ampacity:
            st.markdown("**Cable ampacity**")
            for line in r.ampacity.trace: st.code(line)
        if r.voltage_drop:
            st.markdown("**Voltage drop**")
            for line in r.voltage_drop.trace: st.code(line)

    st.caption("V0 scope is intentionally narrow. IEC 60364-5-52:2009 base-edition data is used only where explicitly supported. Current IEC 60364-4-43 verification is not yet implemented.")
else:
    st.info("Enter a feeder in the sidebar and press **Check feeder**. The defaults are a generic demonstration case, not a design recommendation.")
