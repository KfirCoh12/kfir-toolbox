    ...
    if st.button("Suggest supply", type="primary", use_container_width=True):
        ...
        with st.expander("Why this suggestion?"):
            ...
            if r_al.rejected_candidates:
                ...
                for x in r_al.rejected_candidates: st.write("•",x)
    else:
        st.info("Start with the consumer kW. Add demand factor, voltage, power factor and cable length if known, then press **Suggest supply** — the tool will calculate current and cable options for you.")
