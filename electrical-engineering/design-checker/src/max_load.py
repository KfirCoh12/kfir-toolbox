...
class MaxLoadResult:
    ...
    coverage_status: Literal["FULL CORE COVERAGE", "PARTIAL CORE COVERAGE", "NO USABLE CONSTRAINT"]
...
def calculate_max_load(data: MaxLoadInput) -> MaxLoadResult:
    ...
    limits=[]; limitations=[]; trace=[]; missing_core=[]
    ...
    if data.connection_option_id is not None:
        ...
        if option.rating_a is not None:
            ...
            missing_core.append("The selected fixed connection has no generic numerical current ceiling; equipment/terminal capability still needs verification.")
    ...
    if data.ampacity_route is not None:
        ...
        if amp.iz_a is None:
            limitations.extend(amp.missing_or_unsupported); trace.append("Cable ampacity could not be verified for the supplied conditions.")
            missing_core.append("Cable ampacity could not be verified for the supplied conditions.")
    ...
    if not limits:
        ...
        return MaxLoadResult("NOT VERIFIED",None,None,None,None,tuple(),tuple(dict.fromkeys(missing_core)),"NO USABLE CONSTRAINT",tuple(dict.fromkeys(limitations)),tuple(trace))
    ...
    return MaxLoadResult("RESULT",max_i,max_kw,max_kva,limiting.name,tuple(limits),tuple(dict.fromkeys(missing_core)),coverage,tuple(dict.fromkeys(limitations)),tuple(trace))
