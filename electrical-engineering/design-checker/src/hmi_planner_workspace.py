...
.bp-toolbar{display:flex;align-items:center;justify-content:space-between;gap:.8rem;margin-bottom:.55rem;padding:.42rem .58rem;background:#0a1524;border:1px solid #1d3049;border-radius:8px}.bp-path{font-size:.69rem;color:#8da2bb}.bp-toolbar-note{font-size:.64rem;color:#647d9b}.bp-contextbar{display:flex;align-items:center;justify-content:space-between;gap:.7rem;margin:.15rem 0 .45rem;padding:.35rem .55rem;border-left:3px solid #36a7ff;background:rgba(54,167,255,.055);border-radius:0 7px 7px 0}.bp-contextbar strong{font-size:.68rem;color:#badfff}.bp-contextbar span{font-size:.63rem;color:#718aa7}
...
.bp-selection{display:flex;gap:.5rem;align-items:center;padding:.5rem .62rem;margin:.35rem 0 .58rem;border:1px solid rgba(54,167,255,.27);background:linear-gradient(90deg,rgba(54,167,255,.11),rgba(54,167,255,.035));border-radius:8px}.bp-selection-dot{width:7px;height:7px;border-radius:50%;background:#40b3ff;box-shadow:0 0 0 4px rgba(54,167,255,.1)}.bp-selection-title{font-size:.72rem;color:#cfeaff;font-weight:700}.bp-selection-sub{font-size:.62rem;color:#7895b6}
...
def _selected_graph_nodes(graph, selected: dict | None) -> tuple[str, ...]:
    if selected is None:
...
def render_workspace() -> None:
    ...
    if allowed:
        ...
        with cols[3]:
            if st.button("↺ Reset preview", use_container_width=True):
                st.session_state["hmi_board_sandbox"] = deepcopy(persisted); st.session_state["hmi_selected_uid"] = "root"; st.rerun()
    else:
    ...
    with left:
        ...
        with st.container(border=True):
            ...
            chosen = st.radio("Hierarchy",options,index=options.index(current),format_func=lambda value:labels.get(value,value),label_visibility="collapsed",key="hmi_tree_radio_v2")
            if chosen != st.session_state.get("hmi_selected_uid"):
                st.session_state["hmi_selected_uid"] = chosen; st.rerun()
            st.markdown('<div class="bp-callout">The hierarchy is now the command context: selecting an item controls both its inspector and which downstream additions are available.</div>', unsafe_allow_html=True)

    with center:
        st.markdown('<div class="bp-column-anchor"></div>', unsafe_allow_html=True)
        with st.container(border=True):
            _panel_header("Single-line workspace","Conventional line symbols replace equipment cards while design context stays visible.","LIVE")
            ...
            selection_title = f"{board_id} · {description}" if selected is None else _branch_label(selected).strip(); selection_sub = "Main board / busbar context" if selected is None else f"Selected {selected.get('kind','item').replace('_',' ')}"
            st.markdown(f'<div class="bp-selection"><span class="bp-selection-dot"></span><div><div class="bp-selection-title">{escape(selection_title)}</div><div class="bp-selection-sub">{escape(selection_sub)}</div></div></div>', unsafe_allow_html=True)
            if calculation_error: st.warning(calculation_error)
            elif graph is not None:
                svg = render_hmi_single_line_svg(graph, selected_node_ids=_selected_graph_nodes(graph,selected))
                components.html(f'<div class="bp-sld-shell">{svg}</div>',height=525,scrolling=False)
            st.markdown('<div class="bp-minihead">Calculated circuit schedule</div>',unsafe_allow_html=True)
            schedule=[]
            if calculated is not None:
                for context in calculated.circuit_contexts: schedule.append({"Circuit":context.circuit_id,"Ib":_fmt_a(context.design_current_a),"Breaker":_fmt_rating(context.breaker_candidate_a),"Cable":_fmt_cable(context.cable_mm2,context.cable_runs)})
            st.dataframe(schedule,use_container_width=True,hide_index=True,height=min(250,72+max(1,len(schedule))*35))
    ...
    with right:
        ...
        with st.container(border=True):
            ...
            if selected is None:
                ...
                bid=st.text_input("Board ID",value=board_id,key="hmi2_board_id"); desc=st.text_input("Description",value=description,key="hmi2_board_desc"); v1,v2=st.columns(2)
                with v1: vll=st.number_input("L-L voltage (V)",min_value=1.0,value=float(sandbox.get("line_to_line_voltage_v",400)),step=10.0,key="hmi2_vll")
                with v2: vln=st.number_input("L-N voltage (V)",min_value=1.0,value=float(sandbox.get("line_to_neutral_voltage_v",230)),step=10.0,key="hmi2_vln")
                ...
                if kind=="final":
                    cid=st.text_input("Circuit ID",value=str(selected.get("circuit_id","")),key=f"hmi2_cid_{selected['uid']}"); desc=st.text_input("Description",value=str(selected.get("description","")),key=f"hmi2_desc_{selected['uid']}"); load=st.number_input("Connected load (kW)",min_value=.1,value=float(selected.get("load_kw",5)),step=1.0,key=f"hmi2_load_{selected['uid']}"); a,b=st.columns(2)
                    with a: phase=st.selectbox("Phase",["single","three"],index=0 if selected.get("phase")=="single" else 1,format_func=lambda x:"1P" if x=="single" else "3P",key=f"hmi2_phase_{selected['uid']}")
                    with b: material=st.selectbox("Conductor",["copper","aluminium"],index=1 if selected.get("material")=="aluminium" else 0,format_func=lambda x:"Aluminium" if x=="aluminium" else "Copper",key=f"hmi2_mat_{selected['uid']}")
                    pf=st.number_input("Power factor",min_value=.01,max_value=1.0,value=float(selected.get("power_factor",.9)),step=.01,key=f"hmi2_pf_{selected['uid']}"); demand=st.number_input("Demand factor",min_value=.01,max_value=1.0,value=float(selected.get("demand_factor",1)),step=.05,key=f"hmi2_df_{selected['uid']}")
                    ...
                    feeder=st.text_input("Feeder ID",value=str(selected.get("feeder_id","")),key=f"hmi2_feed_{selected['uid']}"); fid=st.text_input("Field ID",value=str(selected.get("field_id","")),key=f"hmi2_field_{selected['uid']}"); desc=st.text_input("Description",value=str(selected.get("description","")),key=f"hmi2_fdesc_{selected['uid']}"); material=st.selectbox("Feeder conductor",["copper","aluminium"],index=1 if selected.get("material")=="aluminium" else 0,key=f"hmi2_fmat_{selected['uid']}")
                    ...
                    feeder=st.text_input("Feeder ID",value=str(selected.get("feeder_id","")),key=f"hmi2_sbfeed_{selected['uid']}"); sid=st.text_input("Sub-board ID",value=str(selected.get("sub_board_id","")),key=f"hmi2_sbid_{selected['uid']}"); desc=st.text_input("Description",value=str(selected.get("description","")),key=f"hmi2_sbdesc_{selected['uid']}")
                ...
                if st.button("Remove selected item",use_container_width=True):
                    ...
                    sandbox["branches"]=[item for item in branches if item.get("uid") not in descendants]; st.session_state["hmi_selected_uid"]="root"; st.rerun()
    ...
    st.markdown('<div class="bp-callout warn"><b>Design preview</b> · The layout, contextual commands and single-line graphics are still sandboxed. Production Board Planner data is not overwritten from this page.</div>',unsafe_allow_html=True)
