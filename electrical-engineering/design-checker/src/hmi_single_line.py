    ...
    *,
    ...
    if structural:
        ...
        return (
            ...
            f'vector-effect="non-scaling-stroke"{glow}/>'
            f'</g>'
        )
    return (
        f'<line x1="{x1:.1f}" y1="{y1:.1f}" x2="{x2:.1f}" y2="{y2:.1f}" '
...
def _busbar_rail(center_x: float, y: float, half: float, *, active: bool) -> str:
    fill = _ACCENT if active else _STRUCTURAL
    glow = ' filter="url(#glow)"' if active else ""
    return (
        f'<rect x="{center_x - half:.1f}" y="{y - 3.5:.1f}" width="{2 * half:.1f}" height="7" rx="3.5" '
        f'fill="{fill}" data-role="busbar-rail"{glow}/>'
    )
...
def render_hmi_single_line_svg(graph: BoardElectricalGraph, *, selected_node_ids: tuple[str, ...] = ()) -> str:
    ...
    source_stroke = _ACCENT if source_active else _STRUCTURAL
    ...
    if incomer is not None:
        svg.append(
            _breaker_symbol(
                ...
                incomer.label,
                _rating(incomer),
                highlighted=incomer_active or busbar_active,
                structural=True,
            )
        )
    if busbar is not None:
